...
String base64_encode(const unsigned char *bytes_to_encode,
  ...
  String CreateJwt(const char *project_id, const char *audience_id, const char *thing_id, long long int time, NN_DIGIT *priv_key, int jwt_exp_secs) {
    ecc_init();
    // Making jwt token json

    String payload = String("{\"iss\":\"") + project_id + "\""
    + ",\"aud\":\"" + audience_id + "\""
    + ",\"sub\":\"" + thing_id + "\""
    + ",\"iat\":" + (int) time
